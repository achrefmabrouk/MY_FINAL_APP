...
export default function ButtonAppBar() {
  ...
  const isAuth = useSelector((state) => state.Staff.isAuth);
  return (
    <Box  sx={{ flexGrow: 1 }}>
        ...
        {isAuth ? (
          <>
            <Toolbar>
              ...
              {<span>{staff?.role}<br/>
              <span>{staff?.speciality}</span>
              </span>
              }

              <Typography component="div" sx={{ flexGrow: 1 }}></Typography>
              <div style={{font:'inter',weight:'400px',size:'8px',lineHeight:'9.68px',align:'center'}} >
              {<span style={{width:'118px',height:'9px',top:'43px' }}>{staff?.name}
              <br/>
              <br/>
              <span style={{width:'118px',height:'9px',top:'64px' }}>{staff?.email}</span>
              </span> }
              </div>
